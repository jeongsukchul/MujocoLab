...
from mujocolab.utils.function_utils import global_to_body_velocity, get_foot_step
...
from mujocolab.envs.env_config.robot_config import RobotEnvCfg
@dataclass
class UnitreeH1WalkEnvConfig(RobotEnvCfg):
    ...
    gait: str = "jog"
    robot_name: str = "unitree_h1"
    scene_name: str = "scene.xml"

class UnitreeGo2SamplingConfig(SamplingCfg):
    seed: int =  0
    env_name: str = "unitree_h1_loco"
    Nsample: int = 2048
    Hsample: int = 20
    Hnode: int = 5
    Ndiffuse: int = 1
...
@dataclass
class UnitreeH1LocoEnvConfig(RobotEnvCfg):
    kp: Union[float, jax.Array] = field(default_factory=lambda: jnp.array(
        [
            200.0,
            200.0,
